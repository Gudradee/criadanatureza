...
def index():
    ...
    for parc in db.query(models.ParcelaRecebimento).filter(
        models.ParcelaRecebimento.recebido.is_(True),
        models.ParcelaRecebimento.recebido_em.isnot(None),
        extract("year",  models.ParcelaRecebimento.recebido_em) == mes_ano,
        extract("month", models.ParcelaRecebimento.recebido_em) == mes_num,
    ).all():
        d = parc.recebido_em.day
        if 1 <= d <= ultimo_dia:
            fat_dia[d - 1] += float(parc.valor or 0)

    for parc in db.query(models.ParcelaPagamento).filter(
        models.ParcelaPagamento.pago.is_(True),
        models.ParcelaPagamento.pago_em.isnot(None),
        extract("year",  models.ParcelaPagamento.pago_em) == mes_ano,
        extract("month", models.ParcelaPagamento.pago_em) == mes_num,
    ).all():
        d = parc.pago_em.day
        if 1 <= d <= ultimo_dia:
            desp_dia[d - 1] += float(parc.valor or 0)
