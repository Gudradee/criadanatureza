...
def deletar(parceiro_id):
    ...
    return redirect(request.referrer or "/parceiros")

@bp.route("/<int:parceiro_id>/toggle-status", methods=["POST"])
@admin_required
def toggle_status(parceiro_id):
    ...
    if parceiro.status == "ativo":
        ...
        flash(f"Parceiro '{parceiro.nome}' desativado.", "success")
    else:
        parceiro.status = "ativo"
        flash(f"Parceiro '{parceiro.nome}' reativado.", "success")
    db.commit()
    return redirect(request.referrer or "/parceiros")
