...
def _soma_mf(db, tipo, parceiro_id=None, ano=None, mes=None, categoria=None):
    ...
    if ano:
        q = q.filter(extract("year", models.MovimentacaoFinanceira.data) == ano)
        if mes:
            q = q.filter(extract("month", models.MovimentacaoFinanceira.data) == mes)
    if categoria:
        q = q.filter(models.MovimentacaoFinanceira.categoria == categoria)
    return q.scalar() or 0.0

def _soma_despesas(db, ano=None, mes=None, excluir_categorias=None, parceiro_id=None):

    q = db.query(func.sum(models.Despesa.valor))
    if ano:
        q = q.filter(extract("year", models.Despesa.data_competencia) == ano)
        if mes:
            q = q.filter(extract("month", models.Despesa.data_competencia) == mes)
    if excluir_categorias:

        if isinstance(excluir_categorias, str):
            q = q.filter(models.Despesa.categoria != excluir_categorias)
        else:
            q = q.filter(models.Despesa.categoria.notin_(excluir_categorias))
    if parceiro_id:
        q = q.filter(models.Despesa.parceiro_id == parceiro_id)
    return q.scalar() or 0.0

def _soma_despesas_cat(db, categoria, ano=None, mes=None, parceiro_id=None):
...
def _resumo(db, parceiro_id=None, ano=None, mes=None):

    hoje = datetime.now()
    ano = ano or hoje.year
    mes = mes or hoje.month

    AUTO_CATS = ["Custo de Produção", "Comissão Parceiro"]

    te   = _soma_mf(db, "entrada", parceiro_id)
    ts   = _soma_mf(db, "saida",   parceiro_id)
    tcom_mf = _soma_mf(db, "saida", parceiro_id, categoria="Comissão Parceiro")
    tcom_d  = _soma_despesas_cat(db, "Comissão Parceiro", parceiro_id=parceiro_id)
    ...
    td      = _soma_despesas(db, excluir_categorias=AUTO_CATS, parceiro_id=parceiro_id)    if not parceiro_id else 0.0

    ae   = _soma_mf(db, "entrada", parceiro_id, ano=ano)
    asai = _soma_mf(db, "saida",   parceiro_id, ano=ano)
    acom_mf = _soma_mf(db, "saida", parceiro_id, ano=ano, categoria="Comissão Parceiro")
    acom_d  = _soma_despesas_cat(db, "Comissão Parceiro", ano=ano, parceiro_id=parceiro_id)
    acp_mf  = _soma_mf(db, "saida", parceiro_id, ano=ano, categoria="Custo de Produção")
    acp_d   = _soma_despesas_cat(db, "Custo de Produção", ano=ano)                          if not parceiro_id else 0.0
    ad      = _soma_despesas(db, ano=ano, excluir_categorias=AUTO_CATS)                     if not parceiro_id else 0.0

    me   = _soma_mf(db, "entrada", parceiro_id, ano=ano, mes=mes)
    ms   = _soma_mf(db, "saida",   parceiro_id, ano=ano, mes=mes)
    mcom_mf = _soma_mf(db, "saida", parceiro_id, ano=ano, mes=mes, categoria="Comissão Parceiro")
    mcom_d  = _soma_despesas_cat(db, "Comissão Parceiro", ano=ano, mes=mes, parceiro_id=parceiro_id)
    mcp_mf  = _soma_mf(db, "saida", parceiro_id, ano=ano, mes=mes, categoria="Custo de Produção")
    mcp_d   = _soma_despesas_cat(db, "Custo de Produção", ano=ano, mes=mes)                 if not parceiro_id else 0.0
    md      = _soma_despesas(db, ano=ano, mes=mes, excluir_categorias=AUTO_CATS)            if not parceiro_id else 0.0

    def _consolidar(rec, sai, com_mf, com_d, cp_mf, cp_d, despesa_outras):

        cp  = cp_d  if cp_d  > 0 else cp_mf
        com = com_d if com_d > 0 else com_mf

        outras_mf = max(sai - cp_mf - com_mf, 0)
        total     = cp + com + outras_mf + despesa_outras
        lucro     = rec - total
        return cp, com, outras_mf, total, lucro

    tcp, tcom, t_outras, total_desp, lucro_total = _consolidar(te, ts, tcom_mf, tcom_d, tcp_mf, tcp_d, td)
    acp, acom, a_outras, ano_desp,   lucro_ano   = _consolidar(ae, asai, acom_mf, acom_d, acp_mf, acp_d, ad)
    mcp, mcom, m_outras, mes_desp,   lucro_mes   = _consolidar(me, ms, mcom_mf, mcom_d, mcp_mf, mcp_d, md)
    ...
    return {
        ...
        "total_outras_saidas":   round(t_outras, 2),
        "total_despesas_manual": round(td, 2),
        "total_despesas":        round(total_desp, 2),
        "lucro_estimado":        round(lucro_total, 2),

        "ano_entradas":          round(ae, 2),
        "ano_custo_prod":        round(acp, 2),
        "ano_comissoes":         round(acom, 2),
        "ano_outras":            round(a_outras, 2),
        "ano_despesas_manual":   round(ad, 2),
        "ano_despesas":          round(ano_desp, 2),
        "ano_lucro":             round(lucro_ano, 2),

        "mes_atual_entradas":    round(me, 2),
        "mes_atual_saidas":      round(ms, 2),
        "mes_atual_custo_prod":  round(mcp, 2),
        "mes_atual_comissoes":   round(mcom, 2),
        "mes_atual_outras":      round(m_outras, 2),
...
def listar():
    ...
    if parceiro_id:
        ...
        if parceiro_obj:
            ...
            resumo_p = _resumo(db, parceiro_id, ano=ano, mes=mes)
    ...
    if usuario.role == "admin":
        ...
        if chart_produto_id_c:
            ...
            if prod_c:
                ...
                ).filter(models.Despesa.categoria == "Custo de Produção",
                         models.Despesa.descricao.ilike(f"%{prod_c.nome}%"),
                         extract("year", models.Despesa.data_competencia) == chart_ano_c,
                ).group_by("m").all():
    ...
    return render_template(
        ...
        resumo                   = _resumo(db, parceiro_id, ano=ano, mes=mes),
    ...
    if produto_id:
        ...
        if produto:
            for r in db.query(
                ...
                models.Despesa.descricao.ilike(f"%{produto.nome}%"),
                extract("year", models.Despesa.data_competencia) == ano,
            ).group_by("m").all():
...
_MESES_PT_F = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho',
               'Julho','Agosto','Setembro','Outubro','Novembro','Dezembro']
_MESES_ABR_F = ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez']

def _aba_mes_financeiro(wb, db, ano, mes, *, usuario_nome, ponto_id=None,
                        metod_id=None, so_parceiros=False):

    nome_aba = f"{_MESES_ABR_F[mes-1]} {ano}"
    ws = wb.create_sheet(nome_aba[:31])
    b = xls.SheetBuilder(ws)

    periodo = f"{_MESES_PT_F[mes-1]}/{ano}"
    xls.cabecalho_padrao(b, titulo=f"Financeiro — {periodo}",
                         periodo=periodo, usuario=usuario_nome, span=8)

    resumo = _resumo(db, parceiro_id=None, ano=ano, mes=mes)
    b.table(
        ["Indicador", "Valor do mês"],
        [
            ["Receita (entradas)",          resumo["mes_atual_entradas"]],
            ["Custo de produção",           resumo["mes_atual_custo_prod"]],
            ["Comissões de parceiros",      resumo["mes_atual_comissoes"]],
            ["Outras saídas",               resumo["mes_atual_outras"]],
            ["Despesas manuais",            resumo["mes_atual_despesas_manual"]],
            ["Total despesas",              resumo["mes_atual_despesas"]],
        ],
        formats=[None, xls.FMT_BRL],
        total_row=["Lucro do mês", resumo["mes_atual_lucro"]],
    )

    b.title("Transações do mês", span=6)
    transacoes = (
        db.query(models.MovimentacaoFinanceira)
        .options(joinedload(models.MovimentacaoFinanceira.parceiro))
        .filter(
            extract("year",  models.MovimentacaoFinanceira.data) == ano,
            extract("month", models.MovimentacaoFinanceira.data) == mes,
        )
        .order_by(models.MovimentacaoFinanceira.data.desc())
        .all()
    )
    rows = []
    t_ent = t_sai = 0.0
    ...
    if rows:
        b.table(
            ["Data", "Tipo", "Categoria", "Descrição", "Valor", "Parceiro"],
            rows,
            formats=[xls.FMT_DT, None, None, None, xls.FMT_BRL, None],
            total_row=[None, "TOTAIS", None,
                       f"Entradas: R$ {t_ent:,.2f} · Saídas: R$ {t_sai:,.2f}",
                       round(t_ent - t_sai, 2), None],
        )
    else:
        b.row_values(["Nenhuma transação neste mês."])
        b.blank(1)

    b.title("Faturamento por produto", span=4)
    fat_prod = _faturamento_por_produto(
        db, ano=ano, mes=mes, ponto_id=ponto_id, metodo_id=metod_id,
        so_parceiros=so_parceiros,
    )
    if fat_prod:
        rows = [[f["nome"], int(f["total_vendido"]), f["faturamento"], f["percentual"]] for f in fat_prod]
        b.table(
            ["Produto", "Qtd vendida", "Faturamento", "% do total"],
            rows,
            formats=[None, xls.FMT_INT, xls.FMT_BRL, xls.FMT_PCT],
            total_row=["TOTAL",
                       sum(int(f["total_vendido"]) for f in fat_prod),
                       round(sum(f["faturamento"] for f in fat_prod), 2),
                       100.0 if fat_prod else 0],
        )
    else:
        b.row_values(["Nenhuma venda neste mês."])
        b.blank(1)

    b.title("Faturamento por ponto de venda", span=4)
    fat_pt = _faturamento_por_ponto(db, ano=ano, mes=mes, so_parceiros=so_parceiros)
    if fat_pt:
        rows = [[p["ponto_nome"], int(p["total_vendas"]), p["faturamento"], p["percentual"]] for p in fat_pt]
        b.table(
            ["Ponto", "Nº vendas", "Faturamento", "% do total"],
            rows,
            formats=[None, xls.FMT_INT, xls.FMT_BRL, xls.FMT_PCT],
            total_row=["TOTAL",
                       sum(int(p["total_vendas"]) for p in fat_pt),
                       round(sum(p["faturamento"] for p in fat_pt), 2),
                       100.0 if fat_pt else 0],
        )
    else:
        b.row_values(["Nenhuma venda com ponto registrado."])
        b.blank(1)

    b.title("Vendas detalhadas (margem por venda)", span=8)
    produtos_custo = {p.id: p.preco_custo for p in db.query(models.Produto).all()}
    vendas = (
        db.query(models.VendaFinal)
        .options(joinedload(models.VendaFinal.itens), joinedload(models.VendaFinal.ponto_venda))
        .filter(
            extract("year",  models.VendaFinal.data_venda) == ano,
            extract("month", models.VendaFinal.data_venda) == mes,
        )
    ...
    if vendas:
        ...
        tb = td = tl = tc = 0.0
        for vf in vendas:
            custo = sum(i.quantidade * produtos_custo.get(i.produto_id, 0) for i in vf.itens)
            rows.append([
                xls._to_brt(vf.data_venda),
                vf.forma_pagamento or "—",
                vf.ponto_venda.nome if vf.ponto_venda else "—",
                float(vf.valor_total_bruto or 0),
                float(vf.valor_total_desconto or 0),
                float(vf.valor_total_liquido or 0),
                round(custo, 2),
                round(float(vf.valor_total_liquido or 0) - custo, 2),
            ])
            tb += float(vf.valor_total_bruto or 0)
            td += float(vf.valor_total_desconto or 0)
            tl += float(vf.valor_total_liquido or 0)
            tc += custo
        b.table(
            ["Data", "Forma", "Ponto", "Bruto", "Desconto", "Líquido", "Custo estimado", "Margem"],
            rows,
            formats=[xls.FMT_DT, None, None, xls.FMT_BRL, xls.FMT_BRL, xls.FMT_BRL, xls.FMT_BRL, xls.FMT_BRL],
            total_row=["TOTAIS", None, None, round(tb, 2), round(td, 2),
                       round(tl, 2), round(tc, 2), round(tl - tc, 2)],
        )
    else:
        b.row_values(["Nenhuma venda neste mês."])
...
def _aba_anual_financeiro(wb, db, ano, *, usuario_nome):

    ws = wb.create_sheet(f"Anual {ano}", 0)
    b = xls.SheetBuilder(ws)
    xls.cabecalho_padrao(b, titulo=f"Resumo Anual — {ano}",
                         periodo=f"Janeiro a Dezembro de {ano}",
                         usuario=usuario_nome, span=6)

    rows = []
    tot_rec = tot_desp = tot_lucro = 0.0
    for m in range(1, 13):
        r = _resumo(db, parceiro_id=None, ano=ano, mes=m)
        rec   = r["mes_atual_entradas"]
        desp  = r["mes_atual_despesas"]
        lucro = r["mes_atual_lucro"]
    ...
    tot_marg = round(tot_lucro / tot_rec * 100, 1) if tot_rec > 0 else 0.0
    b.table(
        ["Mês", "Receita", "Despesas", "Lucro", "Margem %"],
        rows,
        formats=[None, xls.FMT_BRL, xls.FMT_BRL, xls.FMT_BRL, xls.FMT_PCT],
        total_row=[f"TOTAL {ano}", round(tot_rec, 2), round(tot_desp, 2),
                   round(tot_lucro, 2), tot_marg],
    )
    b.apply_widths()
...
def export_xlsx():
    ...
    ponto_id = request.args.get("ponto_id", type=int)
    metod_id = request.args.get("metodo_id", type=int)
    so_parceiros = request.args.get("so_parceiros") == "1"

    wb = xls.novo_workbook({"title": f"Financeiro {ano}"})
    usuario = get_usuario_atual()
    usuario_nome = usuario.nome if usuario else None

    for m in range(1, 13):
        _aba_mes_financeiro(wb, db, ano, m,
                            usuario_nome=usuario_nome,
                            ponto_id=ponto_id, metod_id=metod_id,
                            so_parceiros=so_parceiros)

    _aba_anual_financeiro(wb, db, ano, usuario_nome=usuario_nome)

    ws = wb.create_sheet("Parceiros")
    b = xls.SheetBuilder(ws)
    xls.cabecalho_padrao(b, titulo="Resumo por parceiro (histórico acumulado)",
                         periodo="Todo o histórico",
                         usuario=usuario_nome, span=5)
    parc_rows = []
    t_rec = t_com = t_emp = 0.0
    for parceiro in db.query(models.Parceiro).filter_by(status="ativo").order_by(models.Parceiro.nome).all():
        receita = db.query(func.sum(models.MovimentacaoFinanceira.valor)).filter(
            models.MovimentacaoFinanceira.tipo == "entrada",
            models.MovimentacaoFinanceira.parceiro_id == parceiro.id,
        ).scalar() or 0.0
        ...
        if receita == 0 and comissoes == 0:
            continue
        empresa = float(receita) - float(comissoes)
        parc_rows.append([
            parceiro.nome,
            round(parceiro.comissao_percentual * 100, 2),
            round(float(receita), 2),
            round(float(comissoes), 2),
            round(empresa, 2),
        ])
        t_rec += float(receita)
        t_com += float(comissoes)
        t_emp += empresa
    if parc_rows:
        b.table(
            ["Parceiro", "% comissão", "Receita gerada", "Comissão", "Empresa recebe"],
            parc_rows,
            formats=[None, xls.FMT_PCT, xls.FMT_BRL, xls.FMT_BRL, xls.FMT_BRL],
            total_row=["TOTAL", None, round(t_rec, 2), round(t_com, 2), round(t_emp, 2)],
        )
    b.apply_widths()

    data = xls.to_response_bytes(wb)
    filename = f"financeiro_{ano}_completo.xlsx"
    return Response(
        data,
