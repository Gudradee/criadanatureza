...
class MovimentacaoEstoque(Base):
    ...
    preco_venda_unitario = Column(Float, nullable=True)
    criado_em  = Column(DateTime(timezone=True), server_default=func.now())
...
class VendaFinal(Base):
    ...
    parcelas_recebimento = relationship("ParcelaRecebimento", back_populates="venda",
                                         cascade="all, delete-orphan",
                                         order_by="ParcelaRecebimento.numero")
...
class ParcelaRecebimento(Base):

    __tablename__ = "parcelas_recebimento"
    ...
    valor       = Column(Float,   nullable=False)
    vencimento  = Column(DateTime(timezone=True), nullable=False)
    recebido    = Column(Boolean, default=False, nullable=False)
    recebido_em = Column(DateTime(timezone=True), nullable=True)
    criado_em   = Column(DateTime(timezone=True), server_default=func.now())

    venda       = relationship("VendaFinal", back_populates="parcelas_recebimento")
